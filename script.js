...
window.addEventListener('load', () => {
    ...
    /* ==========================================================================
       RESUME & PORTFOLIO OVERLAY MODAL CAROUSEL
       ========================================================================== */
    const resumeModal = document.getElementById('resumeModal');
    const viewResumeBtn = document.getElementById('viewResumeBtn');
    const mobileViewResumeBtn = document.getElementById('mobileViewResumeBtn');
    ...
    const resumeSlides = document.querySelectorAll('.resume-slide');
    
    let currentResumePage = 1;
    const totalResumePages = resumeSlides.length;

    const updateResumeViewer = () => {
        resumeSlides.forEach((slide, index) => {
            if (index === currentResumePage - 1) {
        ...
        if (resumePageIndicator) {
            resumePageIndicator.textContent = `Page ${currentResumePage} of ${totalResumePages}`;
        }
    };
    ...
    const openResumeModal = (e) => {
        ...
        if (resumeModal) {
            ...
            currentResumePage = 1; // Start on page 1
            updateResumeViewer();
        }
    };
    ...
    const prevResumePage = () => {
        if (currentResumePage > 1) {
            ...
            currentResumePage = totalResumePages; // Wrap around
        }
        updateResumeViewer();
    };
    ...
    const nextResumePage = () => {
        if (currentResumePage < totalResumePages) {
            ...
            currentResumePage = 1; // Wrap around
        }
        updateResumeViewer();
    };
    ...
    if (mobileViewResumeBtn) mobileViewResumeBtn.addEventListener('click', openResumeModal);
    if (navViewResumeBtn) navViewResumeBtn.addEventListener('click', openResumeModal);
    ...
    if (resumePrevBtn) resumePrevBtn.addEventListener('click', prevResumePage);
    if (resumeNextBtn) resumeNextBtn.addEventListener('click', nextResumePage);
    ...
    // Touch Swipe support for mobile inside modal
    let resumeTouchStartX = 0;
    let resumeTouchEndX = 0;
    const resumeViewerFrame = document.querySelector('.resume-viewer-frame');

    if (resumeViewerFrame) {
        resumeViewerFrame.addEventListener('touchstart', (e) => {
            resumeTouchStartX = e.changedTouches[0].screenX;
        }, { passive: true });

        resumeViewerFrame.addEventListener('touchend', (e) => {
            resumeTouchEndX = e.changedTouches[0].screenX;
            const swipeDistance = resumeTouchEndX - resumeTouchStartX;
            if (swipeDistance > 50) {
